...
def get_path_to_logs_dir() -> str:
    global _path_to_logs_dir
    if _path_to_logs_dir is not None:
        return _path_to_logs_dir

    # check env var MC_DATA_DIR
    env_var = os.environ.get("MC_LOGS_DIR")
    if env_var is not None:
        ...
        if os.path.isdir(env_var):
            _path_to_logs_dir = env_var
            _log.info(f"Using MC_LOGS_DIR: {_path_to_logs_dir}")
            return _path_to_logs_dir
        else:
